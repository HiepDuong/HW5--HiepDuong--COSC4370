...
vector<matches> FindMatches(Mat Template, Mat SampleImage, Mat img, Point2i templateCenter, int windowSize, Mat Map) {
	...
	for (int i = 0; i < Template.rows; i++) {
		for (int j = 0; j < Template.cols; j++) {
			...
			dist = tp1 - tp2;
			dist = dist * dist;
			SSD.ATD(i, j) = sum(dist.mul(ValidMask.mul(GaussianMask)))[0];
			SSD.ATD(i, j) = SSD.ATD(i, j) / TotWeight;// SSD(i,j) = SSD(i,j) + dist*ValidMask(ii,jj)*GaussMask(ii,jj)
		}
	...
	double ErrThreshold = 0.05; //PixelList = all pixels (i,j) where SSD(i,j) <= min(SSD)*(1+ErrThreshold)
...
Mat growImage(const Mat &SampleImage, Mat &Image, int windowSize, Mat &map) {
	double MaxErrThreshold = 0.25;
	while (!allFilled(map)) {
		bool progress = false; // progress = 0
		vector<Point2i> PixelList = GetUnfilledNeighbors(map);
		//cout<<"PixelList.size = "<<PixelList.size()<<endl;
		for (int i = 0; i < PixelList.size(); i++) { // foreach pixel in pixellist do
			vector<matches> BestMatches;
			Mat Template = getNeigborhoodWindow(Image, PixelList[i], windowSize); // Template = GetNeighborhoodWindow(Pixel)
			BestMatches = FindMatches(Template, SampleImage, Image, PixelList[i], windowSize, map);//BestMatches = FindMatches(Template, SampleImage)
			// randomly pick one
			int which = rand() % BestMatches.size(); //BestMatch = RandomPick(BestMatches)
			if (BestMatches[which].err < MaxErrThreshold) {
				...
				map.ATD(PixelList[i].y, PixelList[i].x) = 1.0; //Pixel.value = BestMatch.value
				progress = true;
			}
		}
...
int main() {
	...
	// Extend  gray image.
	Mat img;
	img = imread("download.jpg", CV_LOAD_IMAGE_GRAYSCALE);
